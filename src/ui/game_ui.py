...
class GameUI(tk.Tk, UserInterface):
    ...
    def _animate_complete(self) -> None:
        ...
        # Notify if the market was flushed during setup
        if self.__game.was_market_flushed():
            self.__sidebar.add_log("Market reshuffled due to 3 Wild cards.")
    ...
    def _on_route_clicked(self, route) -> None:
        ...
        player = self.__game.get_player()
        try:
            spent_cards = player.claim_route(route)
            self.__game.discard_train_cards(spent_cards)
            ...
            # Refresh market in case the spent cards triggered an immediate reshuffle
            market_cards = self.__game.get_face_up_deck().get_all_cards()
            self.__sidebar.update_market(market_cards)
    ...
    def _on_market_card_clicked(self, card_index: int) -> None:
        ...
        if self.__game.was_market_flushed():
            self.__sidebar.add_log("Market reshuffled due to 3 Wild cards.")
            self.__sidebar.set_status("Market reshuffled due to 3 Wild cards.")
